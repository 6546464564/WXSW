    ...
    companion object {
        ...
        val dbCallback = object : Callback() {
            ...
            override fun onCreate(db: SupportSQLiteDatabase) {
                // 万象书屋: 不再调用 db.setLocale(Locale.CHINESE)。
                // Room 2.7+ 的多连接池初始化会与 setLocale 的 reconfigureAllConnections
                // 产生 SQLITE_BUSY 竞争(虽内部自愈但会刷错误日志)。
                // 已将所有 DAO 中的 `collate localized` 替换为 `collate nocase`,
                // 不再依赖运行期 locale,从根源消除该竞争。
            }
