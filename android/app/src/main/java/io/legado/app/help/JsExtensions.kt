...
/**
 * 万象书屋: 控制 js 端 `java.toast` / `java.longToast` 弹窗节流。
 *
 * 设计原则: 用户**只需要看到一次**警告 (例如「书山聚合: 请登录后使用」),
 * 同一 (源标签, 消息内容) 一次进程生命周期内最多弹 [MAX_PER_KEY] 次,
 * 之后即使书源在每次搜索 / 翻章节时都调 java.toast(), 用户也不会再被打扰.
 * 进程重启 (冷启动) 后计数清零, 让用户**这次会话**首次仍能看到提示.
 *
 * 这比无止境每 5s 弹一次明显克制 — 实测某些聚合源在并发搜索时一秒内
 * 触发 toast 几十次, 即便有 5s 节流仍会在用户翻页时持续刷屏.
 */
private object JsToastThrottle {
    /** 同一 (tag, msg) 在本进程内最多弹这么多次, 之后静音 */
    private const val MAX_PER_KEY = 1
    private const val CAPACITY = 256

    /**
     * 万象书屋: 用 LinkedHashMap (access-order) 实现 LRU,
     * 避免之前"clear 全部" 让已经 suppress 过的 tag 再次出声 (用户体验差).
     * 超过 CAPACITY 时只 evict 最旧的, 已被静音的 key 仍保持静音.
     * 用 synchronized 替代 ConcurrentHashMap 是因为 LinkedHashMap.access-order 不支持并发.
     */
    private val emitCount = object : LinkedHashMap<String, Int>(CAPACITY, 0.75f, true) {
        override fun removeEldestEntry(eldest: MutableMap.MutableEntry<String, Int>?): Boolean {
            return size > CAPACITY
        }
    }

    fun shouldEmit(tag: String?, msg: Any?): Boolean {
        val key = "${tag ?: ""}|${msg ?: ""}"
        synchronized(emitCount) {
            val cur = (emitCount[key] ?: 0) + 1
            emitCount[key] = cur
            return cur <= MAX_PER_KEY
        }
    }
}
...
//******************文件操作************************//
     ...
     * 弹窗提示
     *
     * 万象书屋: 同一个 (源, 内容) 在 [JS_TOAST_THROTTLE_MS] 毫秒内只弹一次,
     * 避免多个并发搜索/翻章节时被同一书源刷屏 (典型场景: 「书山聚合」未登录)。
     */
    fun toast(msg: Any?) {
        rhinoContext.ensureActive()
        if (JsToastThrottle.shouldEmit(getSource()?.getTag(), msg)) {
