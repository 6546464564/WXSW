...
class ChapterListAdapter(context: Context, val callback: Callback) :
    ...
    override fun convert(
        ...
        binding.run {
            ...
            if (payloads.isEmpty()) {
                upChapterNameColor(binding, item, isDur, cached)
     ...
     * 万象书屋: 章节名颜色三态显示
     * - 当前阅读章节 → accent 色
     * - 已缓存章节 → 主文字色 (primaryText)
     * - 未缓存章节 → 灰色 (secondaryText), 暗示「需联网加载」
     * 卷名特殊处理: 不染灰, 始终主文字色, 避免目录章节标题黯淡
     */
    private fun upChapterNameColor(
        binding: ItemChapterListBinding,
        item: BookChapter,
        isDur: Boolean,
        cached: Boolean
        ...
        val color = when {
            isDur -> context.accentColor
            !cached && !item.isVolume -> context.getCompatColor(R.color.secondaryText)
            else -> context.getCompatColor(R.color.primaryText)
        }
        binding.tvChapterName.setTextColor(color)
    }
     ...
     * 万象书屋: 缓存状态图标
     * - 已缓存 → 绿色对勾 (success 色)
     * - 未缓存 → 灰色云朵 (secondaryText 色), 暗示「在线加载」
     * - 当前阅读章节 → 对勾染 accent 色, 优先级最高
     */
        ...
        binding.apply {
            when {
                isDur -> {
                    ...
                    ivChecked.imageTintList =
                        ColorStateList.valueOf(context.accentColor)
                }
                cached -> {
                    ivChecked.setImageResource(R.drawable.ic_check)
                    ivChecked.imageTintList = ColorStateList.valueOf(
                        context.getCompatColor(R.color.success)
                    )
                }
                else -> {
            ...
            // 已缓存 / 未缓存 / 当前章节 → 总是显示图标 (而不是只在未缓存时显示),
            // 让用户对每条章节都能直接看到状态
            ivChecked.visible()
        }
