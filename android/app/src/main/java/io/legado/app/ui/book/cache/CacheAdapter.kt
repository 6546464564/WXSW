            ...
            if (payloads.isEmpty()) {
                ...
                if (item.isLocal) {
                    ...
                    if (cs == null) {
                        ...
                        // 万象书屋: 分母取「书源章节数」与「实际缓存数」中的较大值,
                        // 避免出现 712/710 这种「已下载>总数」的视觉矛盾 (chapter 表含旧章, 而书源最新数偏小)
                        val total = maxOf(item.totalChapterNum, cs.size)
                        tvDownload.text =
                            context.getString(R.string.download_count, cs.size, total)
                    }
                }
            } else {
                if (item.isLocal) {
                    ...
                    val total = maxOf(item.totalChapterNum, cacheSize)
                    tvDownload.text =
