    ...
    fun startContentSearch(query: String) {
        ...
        searchJob = lifecycleScope.launch(IO) {
            ...
            kotlin.runCatching {
                ...
                // 万象书屋: 统计实际可搜的章节数, 给「0 结果」时一个准确解释
                var scanned = 0
                var skipped = 0
                chapters.forEach { bookChapter ->
                    ensureActive()
                    val canSearch = isLocalBook
                ...
                if (viewModel.searchResultCounts == 0) {
                    val msg = when {
                        scanned == 0 && skipped > 0 ->
                            getString(R.string.search_content_no_cached, skipped)
                        skipped > 0 ->
                            getString(R.string.search_content_partial_empty, scanned, skipped)
                        else -> getString(R.string.search_content_empty)
                    }
                    val noSearchResult = SearchResult(resultText = msg)
                    binding.tvCurrentSearchInfo.post {
