...
/**
 * 万象书屋: 阅读 / 漫画 / 听书 都是 singleTask, 用户从 BookStore → 详情 → 阅读后,
 * 中间的 SearchActivity / BookInfoActivity 都被 singleTask 清掉, 直接 finish 会回桌面.
 * 在调用 super.finish 前先把 MainActivity 拉回栈, 用户 Back 会回到主界面而非桌面.
 */
fun Activity.ensureMainActivityIfTaskRoot() {
    if (isTaskRoot) {
