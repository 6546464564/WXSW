...
object QidianRepository {
     ...
     *
     * 设计变更说明 (2026-04-30 #2):
     *   1. 早期用 /store/c{2|8|10}/... 区分分类, 但 zongheng 在无登录态下对几乎所有 listing 路径
     *      都回退到「最新更新」热榜, 导致三个 Tab 内容相同 (回归测试中复现).
     *   2. 改用 *排序方式 + 完结状态* 区分仍然命中相同热榜.
     *   3. 最终方案: 每个频道拉不同的 page 区间, 利用 zongheng 翻页天然产生不同的书目集合,
     *      在客户端做视觉差异化 (服务端不区分时也能让用户看到不同列表).
     *
     * URL 模式: /store/c0/c0/b{bs}/u0/p{page}/v0/s{sort}/t0/u0/i1/ALL.html
     */
    enum class Channel(
        val label: String,
        val sort: Int,
        val bookStatus: Int,
        /** 该频道实际请求的两个 page 号 (zongheng 一页约 18 本) */
        val pages: IntArray,
    ) {
        Male("男生", sort = 9, bookStatus = 0, pages = intArrayOf(1, 2)),
        Female("女生", sort = 10, bookStatus = 0, pages = intArrayOf(3, 4)),
        Publish("出版", sort = 9, bookStatus = 3, pages = intArrayOf(5, 6)),
        ;

        fun urlFor(page: Int): String =
            "https://book.zongheng.com/store/c0/c0/b$bookStatus/u0/p$page/v0/s$sort/t0/u0/i1/ALL.html"
    }
