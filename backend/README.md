...
| `GET` | `/api/bookstore/mirror` | 书城 Mirror 缓存（后端定时抓 m.qidian.com 写入 DB）；支持 `ETag` / `304`；未就绪时 `503`，App 可降级直抓 |
| `POST` | `/api/ping` | 心跳上报，body `{device_id: "..."}`；用于实时在线 + DAU 统计 |
| `POST` | `/api/source-error` | 解析器错误上报（当前 iOS 已实现），body `{sourceUrl, platform, stage, status, errorMessage, sampleKeyword, sampleUrl, appVer}` |
...
- 功能：
  ...
  - 系统公告（发布 / 启用 / 删除，`GET/POST/DELETE /api/admin/announcement*`）
